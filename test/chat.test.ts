    ...
    default: class MockFuse {
      search(query: string) {
        if (query.includes("project"))
          {return [
            { item: { content: "Desc", title: "Mock Project", type: "project" }, score: 0.1 },
          ];}
        return [];
      }
    },
...
describe("Chat Loader Logic", () => {
  ...
  it("should return default message for empty query", async () => {
    const response = await loader({
      context: {},
      params: {},
      request: createRequest("http://localhost/api/chat"),
    });
  ...
  it("should detect Jibberish", async () => {
    const response = await loader({
      context: {},
      params: {},
      request: createRequest("http://localhost/api/chat?q=asdfasdf"),
    });
    const json = await parseResponse(response);
  ...
  it("should handle Hiring Intent", async () => {
    const response = await loader({
      context: {},
      params: {},
      request: createRequest("http://localhost/api/chat?q=hire%20santiago"),
    });
    const json = await parseResponse(response);
  ...
  it("should handle Creator Question", async () => {
    const response = await loader({
      context: {},
      params: {},
      request: createRequest("http://localhost/api/chat?q=who%20made%20you"),
    });
    const json = await parseResponse(response);
  ...
  it("should handle Security Threats", async () => {
    const response = await loader({
      context: {},
      params: {},
      request: createRequest("http://localhost/api/chat?q=<script>alert(1)</script>"),
    });
    const json = await parseResponse(response);
