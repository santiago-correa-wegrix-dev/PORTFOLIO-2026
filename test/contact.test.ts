...
const mockArgs = (request: Request) =>
  ({ context: {}, params: {}, request }) as unknown as ActionFunctionArgs;
...
describe("Contact Action Logic", () => {
  ...
  it("should accept valid submission", async () => {
    const response = await action(mockArgs(createRequest(validPayload, "10.0.0.1")));
    const json = await parseResponse(response);
  ...
  it("should rate limit after 3 requests from the same IP", async () => {
    ...
    await action(mockArgs(createRequest(validPayload, ip)));
    await action(mockArgs(createRequest(validPayload, ip)));
    await action(mockArgs(createRequest(validPayload, ip)));

    const response = await action(mockArgs(createRequest(validPayload, ip)));
    const json = await parseResponse(response);
  ...
  it("should not rate limit different IPs", async () => {
    ...
    const response = await action(mockArgs(createRequest(validPayload, "172.16.0.2")));
    const json = await parseResponse(response);
